...
import { UserRole, USERS, KycStatus } from "@repo/interfaces";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
                ...
                pipeline: [
                    {
                        $match: {
                            ...
                            "walletProvider.address": { $exists: true, $ne: "" },
                        },
                    },
                    {
                        $project: {
    ...
    const header = "walletAddress,amount";
    const rows = positiveResults.map((row) =>
        [
            toCsvValue(row.walletAddress),
    ...
    res.setHeader(
        "Content-Disposition",
        `attachment; filename="usdc-payout-preview-${safeTimestamp}.csv"`
    );
