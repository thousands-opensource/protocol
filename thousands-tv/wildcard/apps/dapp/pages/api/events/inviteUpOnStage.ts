...
export default async function handler(
    ...
    try {
        ...
        await userRepository.addRoleToUser(userId, UserRole.GUESTBROADCASTER);

        //Send an invite up on stage via PubNub
        const secretKey = getPubnubSecretKey();
        const pubnub = getPubnubInstance("SystemInvite", secretKey);

        const inviteMessage: string = `inviteonstage|${userName}`;
