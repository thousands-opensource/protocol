...
import { fetchNftsForOwnerBase, fetchNftsForOwnerEth } from "@/utils/backend/alchemyUtil";
    ...
    if (walletAddresses) {
        ...
        for (const address of walletAddresses) {
            ...
            const ownedBaseNftsResp = await fetchNftsForOwnerBase(
                address,
                ["0x0aA45c7d5C9ba87912E8603076319009deDa4ca0"] //The Wardens SBT
            );

            if (ownedBaseNftsResp && ownedBaseNftsResp.ownedNfts.length > 0)
            {
                isBaseNftHolder = true;
                break;
            }                         
        }
