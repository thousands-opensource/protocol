...
} from "@snagsolutions/sdk/resources/referral/users";
import {
    sendApiResponse,
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const { walletAddress, loyaltyRuleId, limit, startingAfter, organizationId, websiteId } =
        req.query;
    ...
    if (loyaltyRuleId && !isNonEmptyString(loyaltyRuleId)) {
        sendApiResponseWithStatusCode(
            res,
            {
                success: false,
                err: "loyaltyRuleId must be a string when provided",
            },
            400
        );
        return;
    ...
    try {
        ...
        if (loyaltyRuleId) {
            params.loyaltyRuleId = loyaltyRuleId;
        }
        ...
        const response: UserListResponse = await client.referral.users.list(params);
        ...
        console.error("Failed to fetch Snag Solutions referrals", error);
