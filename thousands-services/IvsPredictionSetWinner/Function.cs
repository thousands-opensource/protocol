...
namespace IvsPredictionSetWinner;
public class PredictionSetWinnerRequest
{
...
public class PredictionSetWinnerResponse
{
...
public class Function
{
    ...
    public Function()
    {
        ...
        _leaderboardRepository = services.GetRequiredService<ILeaderboardRepository>();
        _leaderboardService = services.GetRequiredService<ILeaderboardService>();
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        PredictionSetWinnerRequest? predictionSetWinnerRequest;
        try
        {
            predictionSetWinnerRequest = JsonSerializer.Deserialize<PredictionSetWinnerRequest>(proxyRequest.Body);
            if (predictionSetWinnerRequest == null)
            {
        ...
        string stageId = predictionSetWinnerRequest.StageId;
        string teamName = predictionSetWinnerRequest.TeamName;
        int segment = predictionSetWinnerRequest.Segment;
        ...
        if (segment < 0)
        {
            return ThousandsResponse("ThousandsWarning: Invalid segment", null, 200);
        }
        ...
        try
        {
            SuccessAndErrorMessage successfullyProcessWinner = await _predictionService.SetWinner(stageId, segment, teamName);

            if (!successfullyProcessWinner.Success)
            {
                return ThousandsResponse($"ThousandsWarning: {successfullyProcessWinner.ErrorMessage}", null, 200);
            }

            //Increment the currentSegment on the stage collection after we successfully set the winner
            await _eventRepository.IncrementCurrentSegment(stageId, 1);

            PredictionSetWinnerResponse predictionSetWinnerResponse = new PredictionSetWinnerResponse()
            {
                Success = true
            };
            return ThousandsResponse("ThousandsSuccess: Successfully batch process winners", predictionSetWinnerResponse, 200, true);
        }
        catch (Exception ex)
        {
            return ThousandsResponse($"ThousandsError: Failed to batch process winners - {ex.Message}");
        }
    }

    private APIGatewayProxyResponse ThousandsResponse(string message, PredictionSetWinnerResponse? response = null, int statusCode = 500, bool? success = false)
    {
        Console.WriteLine(message);
        PredictionSetWinnerResponse predictionSetWinnerResponse = response ?? new PredictionSetWinnerResponse()
        {
        ...
        return new APIGatewayProxyResponse
        {
            ...
            Body = JsonSerializer.Serialize(predictionSetWinnerResponse),
