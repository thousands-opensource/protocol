...
namespace IvsQueueGetPosition;

public class GetQueuePositionRequest
{
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        //Get the incoming GetQueuePositionRequest
        GetQueuePositionRequest? getQueuePositionRequest = JsonSerializer.Deserialize<GetQueuePositionRequest>(proxyRequest.Body);
        if (getQueuePositionRequest == null)
        {
            Console.WriteLine("Error deserializing GetQueuePositionRequest!");
            return ReturnResponseError("Error deserializing GetQueuePositionRequest!");
        }

        //Make sure we have a queueId
        if (string.IsNullOrWhiteSpace(getQueuePositionRequest.QueueId))
        {
            Console.WriteLine("Invalid queueId!");
            return ReturnResponseError("Invalid queueId!");
        }

        //Check if we even need to put the user in a queue
        //Get number of available slots in the stream

        PositionInQueue positionInQueue = await _queueService.GetPositionInQueue(getQueuePositionRequest.QueueId, userId);
