...
namespace RallyPredictionsInitiate;

public class InitiatePredictionResponse
{
    public bool Success { get; set; } = false;
    public PredictionPriceQuote? Data { get; set; } = null;
...
public class InitiatePredictionRequest
{
    ...
    public bool Choice { get; set; } = false;
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        string predictionId = initiatePredictionRequest.PredictionId;
        bool choice = initiatePredictionRequest.Choice;
        int credits = initiatePredictionRequest.Credits;

        if (string.IsNullOrWhiteSpace(predictionId))
        {
            return ThousandsResponse("ThousandsWarning: Invalid predictionId", null, 200);
        }

        if ((credits != 0 && credits < 10) || credits > 100000)
        {
            return ThousandsResponse("ThousandsWarning: Credits cannot be less than 10 or greater than 100000", null, 200);
        }

        try
        {
            PredictionPriceQuote predictionPriceQuote = await _predictionService.GetPriceQuote(userId, choice, credits, predictionId);
            ...
            InitiatePredictionResponse initiatePredictionResponse = new InitiatePredictionResponse()
            {
                Data = predictionPriceQuote,
                Success = true
            };
            return ThousandsResponse("ThousandsSuccess: Successfully initiate prediction price quote", initiatePredictionResponse, 200);
    ...
    private APIGatewayProxyResponse ThousandsResponse(string message, InitiatePredictionResponse? response = null, int statusCode = 500)
    {
        Console.WriteLine(message);
        InitiatePredictionResponse initiatePredictionResponse = response ?? new InitiatePredictionResponse()
        {
        ...
        return new APIGatewayProxyResponse
        {
            ...
            Body = JsonSerializer.Serialize(initiatePredictionResponse),
