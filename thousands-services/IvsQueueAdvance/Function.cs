...
namespace IvsQueueAdvance;

public class AdvanceQueueRequest
{
    public string QueueId { get; set; }
    public int AmountToAdvanceTheQueue { get; set; }
}

public class AdvanceQueueResponse
{
    public int NewBeginningOfLine { get; set; }
}
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        if (string.IsNullOrEmpty(userId))
        {
            ...
            return ReturnResponseError("Missing userId!");
        }

        //Get the incoming AdvanceQueueRequest
        AdvanceQueueRequest? advanceQueueRequest = JsonSerializer.Deserialize<AdvanceQueueRequest>(proxyRequest.Body);
        if (advanceQueueRequest == null)
        {
            Console.WriteLine("Error deserializing AdvanceQueueRequest!");
            return ReturnResponseError("Error deserializing AdvanceQueueRequest!");
        }

        //Make sure we have a queueId
        if (string.IsNullOrWhiteSpace(advanceQueueRequest.QueueId))
        {
            Console.WriteLine("Invalid queueId!");
            return ReturnResponseError("Invalid queueId!");
        }

        //Make sure we have a AmountToAdvanceTheQueue
        if (advanceQueueRequest.AmountToAdvanceTheQueue < 1)
        {
            Console.WriteLine("Invalid amountToAdvanceTheQueue!");
            return ReturnResponseError("Invalid amountToAdvanceTheQueue!");
        }

        AdvanceQueueResponse advanceQueueResponse = new AdvanceQueueResponse();
        advanceQueueResponse.NewBeginningOfLine = await _queueService.AdvanceQueue(advanceQueueRequest.QueueId, advanceQueueRequest.AmountToAdvanceTheQueue);

        //Return the price quote
        return new APIGatewayProxyResponse
        {
            StatusCode = 200,
            Body = JsonSerializer.Serialize(advanceQueueResponse),
