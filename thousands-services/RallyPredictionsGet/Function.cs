...
public class GetPredictionResponse
{
    ...
    [JsonPropertyName("predictionStats")]
    public PredictionStats? Data { get; set; } = null;
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        try
        {
            getPredictionRequest = JsonSerializer.Deserialize<GetPredictionRequest>(proxyRequest.Body);
        ...
        string predictionId = getPredictionRequest.PredictionId;

        try
        {
            PredictionStats predictionStats = await _predictionService.GetPrediction(predictionId);
            GetPredictionResponse getPredictionResponse = new GetPredictionResponse()
            {
                Data = predictionStats,
                Success = true
            };
            return ThousandsResponse("ThousandsSuccess: Successfully get prediction", getPredictionResponse, 200);
        }
        catch (Exception ex)
        {
    ...
    private APIGatewayProxyResponse ThousandsResponse(string message, GetPredictionResponse? response = null, int statusCode = 500)
    {
        Console.WriteLine(message);
        GetPredictionResponse confirmPredictionResponse = response ?? new GetPredictionResponse()
        {
