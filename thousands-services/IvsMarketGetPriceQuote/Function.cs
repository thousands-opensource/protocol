...
namespace IvsMarketGetPriceQuote;

public class GetPriceQuoteRequest
{
...
public class Function
{
    ...
    /// </summary>
    /// <param name="proxyRequest">The incoming params to get a price quote.</param>
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        //Get the incoming GetPriceQuoteRequest
        GetPriceQuoteRequest? getPriceQuoteRequest = JsonSerializer.Deserialize<GetPriceQuoteRequest>(proxyRequest.Body);
        if (getPriceQuoteRequest == null)
        {
            Console.WriteLine("Error deserializing GetPriceQuoteRequest!");
            return ReturnResponseError();
        }

        //Make sure we have a coinName
        if (string.IsNullOrWhiteSpace(getPriceQuoteRequest.CoinName))
        {
            Console.WriteLine("Invalid coinName!");
            return ReturnResponseError();
        }
        //Make sure we have a quantity that is in range
        if (getPriceQuoteRequest.Quantity < 1 || getPriceQuoteRequest.Quantity > 1000)
        {
            Console.WriteLine("Quantity is out of range!");
            return ReturnResponseError();
        }
        //Make sure we have the correct order type
        if (getPriceQuoteRequest.OrderType != "buy" && getPriceQuoteRequest.OrderType != "sell")
        {
            Console.WriteLine("Invalid order type!");
            return ReturnResponseError();
        }

        //x^2 is the bonding curve formula for the market trading system
        string priceFormula = "x^2";

        getPriceQuoteRequest.CoinName = getPriceQuoteRequest.CoinName.ToUpper();
        if (getPriceQuoteRequest.CoinName.StartsWith("BUTTON_"))
        {
            //1/x^n is the formula for the Wildcard idle game
            priceFormula = "1/x^n";
        }

        //Get the price quote from the service
        PriceQuote priceQuote = await _marketService.GetPriceQuote(userId, getPriceQuoteRequest.CoinName, getPriceQuoteRequest.Quantity, getPriceQuoteRequest.OrderType, priceFormula);

        //Return the price quote
        return new APIGatewayProxyResponse
        {
            StatusCode = 200,
            Body = JsonSerializer.Serialize(priceQuote),
