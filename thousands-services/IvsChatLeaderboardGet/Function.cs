...
namespace IvsChatLeaderboardGet;

public class ChatLeaderboardGetRequest
{
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        //Get the incoming ChatLeaderboardGetRequest
        ChatLeaderboardGetRequest? chatLeaderboardGetRequest = JsonSerializer.Deserialize<ChatLeaderboardGetRequest>(proxyRequest.Body);
        if (chatLeaderboardGetRequest == null)
        {
            Console.WriteLine("Error deserializing ChatLeaderboardGetRequest!");
            return ReturnResponseError();
        }
        if (string.IsNullOrEmpty(chatLeaderboardGetRequest.EventId))
        {
            Console.WriteLine("Missing EventId in ChatLeaderboardGetRequest!");
            return ReturnResponseError();
        }
        if (string.IsNullOrEmpty(chatLeaderboardGetRequest.VendorEventId))
        {
            Console.WriteLine("Missing VendorEventId in ChatLeaderboardGetRequest!");
            return ReturnResponseError();
        }

        var chatLeaderboard = await _leaderboardService.GetScores(chatLeaderboardGetRequest.EventId, chatLeaderboardGetRequest.VendorEventId, userId);

        //Return the list of top coins
        return new APIGatewayProxyResponse
        {
            StatusCode = 200,
            Body = JsonSerializer.Serialize(chatLeaderboard),
