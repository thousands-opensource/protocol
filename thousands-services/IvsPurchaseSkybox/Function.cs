...
namespace IvsPurchaseSkybox;

public class PurchaseSkyboxResponse
{
    [JsonPropertyName("success")]
    public bool Success { get; set; }
    [JsonPropertyName("data")]
    public PurchaseSkyboxResult? Data { get; set; }
    [JsonPropertyName("errorMessage")]
    public string? ErrorMessage { get; set; }
}
public class PurchaseSkyboxRequest
{
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        string[] role = thousandsJwt.Roles;
        bool isModerator = role.Contains("moderator");
        ...
        PurchaseSkyboxRequest? purchaseSkyboxRequest;
        try
        {
            purchaseSkyboxRequest = JsonSerializer.Deserialize<PurchaseSkyboxRequest>(proxyRequest.Body);
            if (purchaseSkyboxRequest == null)
            {
        ...
        string stageId = purchaseSkyboxRequest.StageId;
        int skyboxTier = purchaseSkyboxRequest.SkyboxTier;

        if (String.IsNullOrEmpty(stageId))
        {
            return ThousandsResponse($"Invalid stageId", $"ThousandsWarning: Invalid stageId - {stageId}", null, 200);
        }
        if (skyboxTier < 1 || skyboxTier > 3)
        {
            return ThousandsResponse($"Invalid skybox tier", $"ThousandsWarning: Invalid skybox tier - {skyboxTier}", null, 200);
        }

        try
        {
            PurchaseSkyboxResult purchaseSkyboxResult = await _skyboxService.PurchaseSkybox(
                userId,
                stageId,
                skyboxTier, isModerator);

            string? uiMessage = purchaseSkyboxResult.ErrorMessage;
            if (!string.IsNullOrEmpty(uiMessage))
            {
                return ThousandsResponse(uiMessage, $"ThousandsError: Failed to purchase skybox - stageId: {stageId}, skyboxTier: {skyboxTier}", null, 200);
            }

            PurchaseSkyboxResponse purchaseSkyboxResponse = new PurchaseSkyboxResponse()
            {
                Data = purchaseSkyboxResult,
                Success = true
            };
            return ThousandsResponse("", "ThousandsSuccess: Successfully purchased skybox", purchaseSkyboxResponse, 200);
        }
        catch (Exception ex)
        {
            return ThousandsResponse($"Failed to purchase skybox", $"ThousandsError - Failed to purchase skybox - {ex.Message}");
        }
    }

    private APIGatewayProxyResponse ThousandsResponse(string uiMessage, string logMessage, PurchaseSkyboxResponse? response = null, int statusCode = 500)
    {
        Console.WriteLine(logMessage);
        PurchaseSkyboxResponse purchaseSkyboxResponse = response ?? new PurchaseSkyboxResponse()
        {
        ...
        return new APIGatewayProxyResponse
        {
            ...
            Body = JsonSerializer.Serialize(purchaseSkyboxResponse),
