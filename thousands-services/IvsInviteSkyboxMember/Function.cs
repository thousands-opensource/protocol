...
namespace IvsInviteSkyboxMember;

public class InviteSkyboxMemberResponse
{
...
public class InviteSkyboxMemberRequest
{
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        InviteSkyboxMemberRequest? inviteSkyboxMemberRequest;
        try
        {
            inviteSkyboxMemberRequest = JsonSerializer.Deserialize<InviteSkyboxMemberRequest>(proxyRequest.Body);
            if (inviteSkyboxMemberRequest == null)
            {
        ...
        string skyboxId = inviteSkyboxMemberRequest.SkyboxId;
        string memberUserId = inviteSkyboxMemberRequest.MemberUserId;
        string stageId = inviteSkyboxMemberRequest.StageId;
        ...
        try
        {
            FrontEndLogResult inviteSkyboxMemberResult = await _skyboxService.InviteMemberToSkybox(
                userId,
                memberUserId,
                stageId,
                skyboxId
            ...
            string? uiMessage = inviteSkyboxMemberResult.ErrorMessage;
            if (!string.IsNullOrEmpty(uiMessage))
            {
                return ThousandsResponse(uiMessage, $"ThousandsError: Failed to invite a member to skybox {skyboxId}, memberUserId {memberUserId}", null, 200);
            }

            InviteSkyboxMemberResponse inviteSkyboxMemberResponse = new InviteSkyboxMemberResponse()
            {
                Data = true,
                Success = true
            };
            return ThousandsResponse("", $"ThousandsSuccess: Successfully invited a member to skybox userId - {memberUserId}", inviteSkyboxMemberResponse, 200);
        }
        catch (Exception ex)
        {
            return ThousandsResponse($"Failed to invite a member to skybox", $"ThousandsError - Failed to invite a member to skybox - {ex.Message}");
        }
    }

    private APIGatewayProxyResponse ThousandsResponse(string uiMessage, string logMessage, InviteSkyboxMemberResponse? response = null, int statusCode = 500)
    {
        Console.WriteLine(logMessage);
        InviteSkyboxMemberResponse inviteSkyboxMemberResponse = response ?? new InviteSkyboxMemberResponse()
        {
        ...
        return new APIGatewayProxyResponse
        {
            ...
            Body = JsonSerializer.Serialize(inviteSkyboxMemberResponse),
