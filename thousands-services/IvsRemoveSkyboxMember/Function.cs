...
namespace IvsRemoveSkyboxMember;

public class RemoveSkyboxMemberResponse
{
...
public class RemoveSkyboxMemberRequest
{
...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        RemoveSkyboxMemberRequest? removeSkyboxMemberRequest;
        try
        {
            removeSkyboxMemberRequest = JsonSerializer.Deserialize<RemoveSkyboxMemberRequest>(proxyRequest.Body);
            if (removeSkyboxMemberRequest == null)
            {
        ...
        string skyboxId = removeSkyboxMemberRequest.SkyboxId;
        string memberUserId = removeSkyboxMemberRequest.MemberUserId;
        string stageId = removeSkyboxMemberRequest.StageId;
        ...
        {
            return ThousandsResponse($"Invalid memberUserId", $"ThousandsWarning: Invalid memberUserId - {memberUserId}", null, 200);
        }
        if (String.IsNullOrEmpty(stageId))
        {
            return ThousandsResponse($"Invalid stageId", $"ThousandsWarning: Invalid stageId - {stageId}", null, 200);
        }

        try
        {
            RemoveUserResult removeMemberFromSkyboxResult = await _skyboxService.RemoveMemberFromSkybox(userId, memberUserId, stageId, skyboxId); ;

            string? uiMessage = removeMemberFromSkyboxResult.ErrorMessage;
            if (!string.IsNullOrEmpty(uiMessage))
            {
                return ThousandsResponse(uiMessage, $"ThousandsError: Failed to remove skybox member - memberUserId:{memberUserId}", null, 200);
            }

            RemoveSkyboxMemberResponse removeSkyboxMemberResponse = new RemoveSkyboxMemberResponse()
            {
                Data = removeMemberFromSkyboxResult,
                Success = true
            };
            return ThousandsResponse("", $"ThousandsSuccess: Successfully removed skybox member - memberUserId {memberUserId}", removeSkyboxMemberResponse, 200);
        }
        catch (Exception ex)
        {
            return ThousandsResponse($"Failed to remove skybox member", $"ThousandsError - Failed to remove skybox member - {ex.Message}");
        }
    }

    private APIGatewayProxyResponse ThousandsResponse(string uiMessage, string logMessage, RemoveSkyboxMemberResponse? response = null, int statusCode = 500)
    {
        Console.WriteLine(logMessage);
        RemoveSkyboxMemberResponse removeSkyboxMemberResponse = response ?? new RemoveSkyboxMemberResponse()
        {
