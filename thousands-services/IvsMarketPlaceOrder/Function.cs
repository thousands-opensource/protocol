...
public class Function
{
    ...
    /// </summary>
    /// <param name="proxyRequest">The incoming params to place and order.</param>;
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest proxyRequest, ILambdaContext context)
    {
        ...
        //Get the incoming PlaceOrderRequest
        PlaceOrderRequest? placeOrderRequest = JsonSerializer.Deserialize<PlaceOrderRequest>(proxyRequest.Body);
        if (placeOrderRequest == null)
        {
            Console.WriteLine("Error deserializing PlaceOrderRequest!");
            return ReturnResponseError();
        }
        if (string.IsNullOrEmpty(placeOrderRequest.EventId))
        {
            Console.WriteLine("Missing EventId in PlaceOrderRequest!");
            return ReturnResponseError();
        }

        PlaceOrderResult placeOrderResult = await _marketService.PlaceOrder(placeOrderRequest.EventId, userId, placeOrderRequest.PriceQuoteGuid, placeOrderRequest.CoinName,
            placeOrderRequest.Quantity, placeOrderRequest.OrderType, "x^2");

        return new APIGatewayProxyResponse
        {
            StatusCode = 200,
            Body = JsonSerializer.Serialize(placeOrderResult),
